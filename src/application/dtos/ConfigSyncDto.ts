import {
  CreditCurrencyConfigSchema,
  CreditRuleConfigSchema,
  PlanGrantConfigSchema,
  AddonConfigSchema,
  FeatureResolutionConfigSchema,
  SubscriptionOverrideConfigSchema,
} from "./AccountingConfigDto.js";
import { z } from "zod";
 ...
 */
export const BillingCycleConfigSchema = CreateBillingCycleObjectSchema.omit({
  planKey: true,
})
  .extend({
  ...
  .refine(
    ...
    {
      message:
        "Duration value is required for non-forever durations, and must be undefined for forever duration",
      path: ["durationValue"],
    },
  );
 ...
 */
export const PlanConfigSchema = CreatePlanDtoSchema.omit({ productKey: true }) // Remove productKey (comes from nested structure)
  .extend({
    archived: z.boolean().optional(),
    creditGrants: z.array(PlanGrantConfigSchema).optional(),
...
export const FeatureConfigSchema = BaseFeatureDtoSchema.extend({
  creditConsumptionRules: z.array(CreditRuleConfigSchema).optional(),
  archived: z.boolean().optional(),
}).refine(
  (data) => {
    return FeatureValueValidator.isValid(
      data.defaultValue,
      data.valueType as FeatureValueType,
    );
  },
  {
    message:
      'Invalid default value for the selected value type. Toggle must be "true" or "false", Numeric must be a valid number.',
    path: ["defaultValue"],
  },
);
...
export const ProductConfigSchema = CreateProductDtoSchema.extend({
  addons: z.array(AddonConfigSchema).optional(),
  featureResolution: z
    .record(z.string(), FeatureResolutionConfigSchema)
    .optional(),
  archived: z.boolean().optional(),
...
export interface ConfigSyncDto {
  creditConsumptionRules?: Array<
    z.infer<typeof CreditRuleConfigSchema> & { featureKey: string }
  >;
  creditCurrencies?: z.infer<typeof CreditCurrencyConfigSchema>[];
  subscriptions?: z.infer<typeof SubscriptionOverrideConfigSchema>[];
...
export interface ConfigSyncReport {
  details?: {
    ...
    removed: number;
    unchanged: number;
    changes: Array<{ entityType: string; key: string; action: string }>;
  };
  created: {
    features: number;
    products: number;
  ...
  errors: Array<{
    entityType: "feature" | "product" | "plan" | "billingCycle" | "entitlement";
    key: string;
    message: string;
  }>;
  warnings: Array<{
...
function validateJsonPropertyOrder(jsonString: string): void {
  ...
  if (featuresIndex > productsIndex) {
    throw new Error(
      'The "features" array must appear before the "products" array in the JSON configuration',
    );
  }
}
 ...
 */
export const ConfigSyncDtoSchema = z
  .object({
    creditConsumptionRules: z
      .array(CreditRuleConfigSchema.extend({ featureKey: z.string().min(1) }))
      .optional(),
    creditCurrencies: z.array(CreditCurrencyConfigSchema).optional(),
    subscriptions: z.array(SubscriptionOverrideConfigSchema).optional(),
      ...
      if (product.plans) {
        ...
        for (const plan of product.plans) {
              ...
              if (feature) {
                ...
                if (
                  !FeatureValueValidator.isValid(
                    value,
                    feature.valueType as FeatureValueType,
                  )
                ) {
                  ctx.addIssue({
                    code: z.ZodIssueCode.custom,
          ...
          if (plan.onExpireTransitionToBillingCycleKey) {
            if (
              !allProductBillingCycleKeys.has(
                plan.onExpireTransitionToBillingCycleKey,
              )
            ) {
              ctx.addIssue({
                code: z.ZodIssueCode.custom,
