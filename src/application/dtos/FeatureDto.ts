...
export const BaseFeatureDtoSchema = z.object({
  key: z
    ...
    .regex(
      /^[a-zA-Z0-9-_]+$/,
      "Key must be alphanumeric with hyphens/underscores",
    ),
  displayName: z
    .string()
  ...
  valueType: z.enum(["toggle", "numeric", "text", "metered"]),
  defaultValue: z.string().min(1, "Default value is required"),
  groupName: z.string().max(255).optional(),
  meteredConfig: MeteredConfigSchema.optional(),
  validator: z.record(z.string(), z.unknown()).optional(),
  metadata: z.record(z.string(), z.unknown()).optional(),
});

export const CreateFeatureDtoSchema = BaseFeatureDtoSchema.refine(
  (data) => {
...
export const UpdateFeatureDtoSchema = z
  .object({
    ...
    displayName: z
      ...
      .optional(),
    description: z.string().max(1000).optional(),
    valueType: z.enum(["toggle", "numeric", "text", "metered"]).optional(),
...
export interface FeatureDto {
  ...
  meteredConfig?: MeteredFeatureConfigDto | null;
  key: string;
  displayName: string;
