...
export function postgresSslOption(ssl?: boolean): { rejectUnauthorized: true } | undefined {
...
export function initializeDatabase(config: SubscrioConfig['database']): DrizzleDb {
  ...
  const dialect = config.databaseType ?? detectDatabaseDialect(config.connectionString);
