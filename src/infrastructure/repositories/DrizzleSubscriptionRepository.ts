...
export class DrizzleSubscriptionRepository implements ISubscriptionRepository {
  ...
  /** @internal */ coordinateSave?: (
    subscription: Subscription,
    save: () => Promise<Subscription>,
  ) => Promise<Subscription>;

  async save(subscription: Subscription): Promise<Subscription> {
    return this.coordinateSave
      ? this.coordinateSave(subscription, () => this.saveRecord(subscription))
      : this.saveRecord(subscription);
  }
  private async saveRecord(subscription: Subscription): Promise<Subscription> {
    ...
    if (subscription.id === undefined) {
      ...
      if (subscription.props.featureOverrides.length > 0) {
        const overrideRecords = subscription.props.featureOverrides.map(
          (override) => ({
        ...
        await this.db
          .insert(subscription_feature_overrides)
          .values(overrideRecords);
      }
    } else {
      ...
      await this.db
        .delete(subscription_feature_overrides)
        .where(
          eq(subscription_feature_overrides.subscription_id, subscription.id),
        );
  ...
  private async loadFeatureOverrides(
    ...
    const records = await this.db
      ...
      .where(
        eq(subscription_feature_overrides.subscription_id, subscriptionId),
      );
  ...
  async findAll(
  ...
  ): Promise<Array<{ subscription: Subscription; customer: Customer | null }>> {
    ...
    let query = this.db
      ...
      .innerJoin(
        customers,
        eq(subscriptionStatusView.customer_id, customers.id),
      );
    ...
    if (filters) {
      ...
      if ((filters as any).customerId) {
        conditions.push(
          eq(subscriptionStatusView.customer_id, (filters as any).customerId),
        );
      }
      ...
      if ((filters as any).planIds && (filters as any).planIds.length > 0) {
        conditions.push(
          inArray(subscriptionStatusView.plan_id, (filters as any).planIds),
        );
      } else if ((filters as any).planId) {
        conditions.push(
          eq(subscriptionStatusView.plan_id, (filters as any).planId),
        );
      }
      ...
      if ((filters as any).billingCycleId) {
        conditions.push(
          eq(
            subscriptionStatusView.billing_cycle_id,
            (filters as any).billingCycleId,
          ),
        );
      }
      ...
      if (filters.isArchived !== undefined) {
        conditions.push(
          eq(subscriptionStatusView.is_archived, filters.isArchived),
        );
      }
      ...
      if ((filters as any).activationDateFrom) {
        conditions.push(
          gte(
            subscriptionStatusView.activation_date,
            (filters as any).activationDateFrom,
          ),
        );
      }
      if ((filters as any).activationDateTo) {
        conditions.push(
          lte(
            subscriptionStatusView.activation_date,
            (filters as any).activationDateTo,
          ),
        );
      }
      if ((filters as any).expirationDateFrom) {
        conditions.push(
          gte(
            subscriptionStatusView.expiration_date,
            (filters as any).expirationDateFrom,
          ),
        );
      }
      if ((filters as any).expirationDateTo) {
        conditions.push(
          lte(
            subscriptionStatusView.expiration_date,
            (filters as any).expirationDateTo,
          ),
        );
      }
      if ((filters as any).trialEndDateFrom) {
        conditions.push(
          gte(
            subscriptionStatusView.trial_end_date,
            (filters as any).trialEndDateFrom,
          ),
        );
      }
      ...
      if ((filters as any).currentPeriodStartFrom) {
        conditions.push(
          gte(
            subscriptionStatusView.current_period_start,
            (filters as any).currentPeriodStartFrom,
          ),
        );
      }
      if ((filters as any).currentPeriodStartTo) {
        conditions.push(
          lte(
            subscriptionStatusView.current_period_start,
            (filters as any).currentPeriodStartTo,
          ),
        );
      }
      if ((filters as any).currentPeriodEndFrom) {
        conditions.push(
          gte(
            subscriptionStatusView.current_period_end,
            (filters as any).currentPeriodEndFrom,
          ),
        );
      }
      if ((filters as any).currentPeriodEndTo) {
        conditions.push(
          lte(
            subscriptionStatusView.current_period_end,
            (filters as any).currentPeriodEndTo,
          ),
        );
      }
      ...
      if ((filters as any).hasStripeId !== undefined) {
        if ((filters as any).hasStripeId) {
          conditions.push(
            isNotNull(subscriptionStatusView.stripe_subscription_id),
          );
        } else {
          conditions.push(
            isNull(subscriptionStatusView.stripe_subscription_id),
          );
        }
      }
      ...
      if (filters.status) {
        conditions.push(
          eq(subscriptionStatusView.computed_status, filters.status),
        );
      }
      ...
      if (sortBy === "activationDate") {
        query = query.orderBy(
          sortOrder === "desc"
            ? desc(subscriptionStatusView.activation_date)
            : asc(subscriptionStatusView.activation_date),
        ) as typeof query;
      } else if (sortBy === "expirationDate") {
        query = query.orderBy(
          sortOrder === "desc"
            ? desc(subscriptionStatusView.expiration_date)
            : asc(subscriptionStatusView.expiration_date),
        ) as typeof query;
      } else if (sortBy === "currentPeriodStart") {
        query = query.orderBy(
          sortOrder === "desc"
            ? desc(subscriptionStatusView.current_period_start)
            : asc(subscriptionStatusView.current_period_start),
        ) as typeof query;
      } else if (sortBy === "currentPeriodEnd") {
        query = query.orderBy(
          sortOrder === "desc"
            ? desc(subscriptionStatusView.current_period_end)
            : asc(subscriptionStatusView.current_period_end),
        ) as typeof query;
      } else {
        query = query.orderBy(
          sortOrder === "desc"
            ? desc(subscriptionStatusView.created_at)
            : asc(subscriptionStatusView.created_at),
        ) as typeof query;
      }
  ...
  async findByCustomerId(
    ...
    for (const record of records) {
      const featureOverrides = await this.loadFeatureOverrides(record.id);
      subscriptionsWithOverrides.push(
        SubscriptionMapper.toDomain(record, featureOverrides),
      );
    }
  ...
  async findActiveByCustomerAndPlan(
    customerId: number,
    planId: number,
  ...
  async hasSubscriptionsForBillingCycle(
  ...
  async findExpiredWithTransitionPlans(
    limit: number = 1000,
  ): Promise<Subscription[]> {
