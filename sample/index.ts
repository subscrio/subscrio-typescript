...
async function runPhase1_SystemSetup(subscrio: Subscrio) {
  ...
  if (isInteractiveMode) {
    await promptForDatabaseInspection(
      "Phase 1: System Setup",
      "Step 1: Schema Installation",
    );
  }
  ...
  if (isInteractiveMode) {
    await promptForDatabaseInspection(
      "Phase 1: System Setup",
      "Step 2: Product Creation Complete",
    );
  }
  ...
  for (const featureData of features) {
    console.log(
      `📥 Input: subscrio.features.createFeature(${JSON.stringify(featureData, null, 2)})`,
    );
    const feature = await subscrio.features.createFeature(featureData);

    console.log(
      `📥 Input: subscrio.products.associateFeature("projecthub", "${feature.key}")`,
    );
    ...
    console.log(
      `📄 Output: Feature DTO (${feature.key}):`,
      JSON.stringify(fetchedFeature, null, 2),
    );
  }
  printDivider();
  ...
  if (isInteractiveMode) {
    await promptForDatabaseInspection(
      "Phase 1: System Setup",
      "Step 3: Features Creation Complete",
    );
  }
  ...
  printInfo(
    "Create each plan with its billing cycles and optional transition configuration",
    1,
  );
  ...
  console.log(
    '📥 Input: subscrio.billingCycles.getBillingCycle("free-forever")',
  );
  console.log(
    `📄 Output: Free Plan DTO:`,
    JSON.stringify(await subscrio.plans.getPlan("free"), null, 2),
  );
  console.log(
    `📄 Output: Free Forever Billing Cycle DTO:`,
    JSON.stringify(
      await subscrio.billingCycles.getBillingCycle("free-forever"),
      null,
      2,
    ),
  );
  ...
  printSuccess(
    `Created plan: ${starterPlan.displayName} (${starterPlan.key}) with auto-transition to free plan`,
  );
  ...
  console.log(
    '📥 Input: subscrio.billingCycles.getBillingCycle("starter-monthly")',
  );
  console.log(
    '📥 Input: subscrio.billingCycles.getBillingCycle("starter-annual")',
  );
  console.log(
    `📄 Output: Starter Plan DTO:`,
    JSON.stringify(await subscrio.plans.getPlan("starter"), null, 2),
  );
  console.log(
    `📄 Output: Starter Monthly Billing Cycle DTO:`,
    JSON.stringify(
      await subscrio.billingCycles.getBillingCycle("starter-monthly"),
      null,
      2,
    ),
  );
  console.log(
    `📄 Output: Starter Annual Billing Cycle DTO:`,
    JSON.stringify(
      await subscrio.billingCycles.getBillingCycle("starter-annual"),
      null,
      2,
    ),
  );
  ...
  printSuccess(
    `Created plan: ${professionalPlan.displayName} (${professionalPlan.key}) with auto-transition to free plan`,
  );
  ...
  console.log(
    '📥 Input: subscrio.billingCycles.getBillingCycle("professional-monthly")',
  );
  console.log(
    '📥 Input: subscrio.billingCycles.getBillingCycle("professional-annual")',
  );
  console.log(
    `📄 Output: Professional Plan DTO:`,
    JSON.stringify(await subscrio.plans.getPlan("professional"), null, 2),
  );
  console.log(
    `📄 Output: Professional Monthly Billing Cycle DTO:`,
    JSON.stringify(
      await subscrio.billingCycles.getBillingCycle("professional-monthly"),
      null,
      2,
    ),
  );
  console.log(
    `📄 Output: Professional Annual Billing Cycle DTO:`,
    JSON.stringify(
      await subscrio.billingCycles.getBillingCycle("professional-annual"),
      null,
      2,
    ),
  );
  ...
  printSuccess(
    `Created plan: ${enterprisePlan.displayName} (${enterprisePlan.key}) with auto-transition to free plan`,
  );
  ...
  console.log(
    '📥 Input: subscrio.billingCycles.getBillingCycle("enterprise-monthly")',
  );
  console.log(
    '📥 Input: subscrio.billingCycles.getBillingCycle("enterprise-annual")',
  );
  console.log(
    `📄 Output: Enterprise Plan DTO:`,
    JSON.stringify(await subscrio.plans.getPlan("enterprise"), null, 2),
  );
  console.log(
    `📄 Output: Enterprise Monthly Billing Cycle DTO:`,
    JSON.stringify(
      await subscrio.billingCycles.getBillingCycle("enterprise-monthly"),
      null,
      2,
    ),
  );
  console.log(
    `📄 Output: Enterprise Annual Billing Cycle DTO:`,
    JSON.stringify(
      await subscrio.billingCycles.getBillingCycle("enterprise-annual"),
      null,
      2,
    ),
  );
  ...
  if (isInteractiveMode) {
    await promptForDatabaseInspection(
      "Phase 1: System Setup",
      "Step 4: Plans Creation Complete",
    );
  }
  ...
  printSuccess(
    "Professional plan: 25 projects, 50 users per project, Gantt charts enabled",
  );
  ...
  if (isInteractiveMode) {
    await promptForDatabaseInspection(
      "Phase 1: System Setup",
      "Step 4: Plans, Billing Cycles, and Transitions Complete",
    );
  }
}
...
async function runPhase2_TrialStart(subscrio: Subscrio) {
  ...
  console.log(
    "📄 Output: Customer DTO:",
    JSON.stringify(fetchedCustomer, null, 2),
  );
  printDivider();
  ...
  if (isInteractiveMode) {
    await promptForDatabaseInspection(
      ...
      "Step 1: Customer Creation Complete",
    );
  }
  ...
  console.log(
    '📥 Input: subscrio.subscriptions.getSubscription("acme-subscription")',
  );
  const fetchedSubscription = await subscrio.subscriptions.getSubscription(
    subscription.key,
  );
  console.log(
    "📄 Output: Trial Subscription DTO:",
    JSON.stringify(fetchedSubscription, null, 2),
  );
  ...
  if (isInteractiveMode) {
    await promptForDatabaseInspection(
      "Phase 2: Customer Onboarding",
      "Step 2: Trial Subscription Creation Complete",
    );
  }
  ...
  const hasGanttCharts = await subscrio.featureChecker.isEnabledForCustomer(
    customerKey,
    "projecthub",
    "gantt-charts",
  );
  const hasCustomBranding = await subscrio.featureChecker.isEnabledForCustomer(
    customerKey,
    "projecthub",
    "custom-branding",
  );
  const hasApiAccess = await subscrio.featureChecker.isEnabledForCustomer(
    customerKey,
    "projecthub",
    "api-access",
  );
  ...
  printInfo(
    `Custom branding: ${hasCustomBranding ? "Enabled" : "Disabled"}`,
    1,
  );
  ...
  if (isInteractiveMode) {
    await promptForDatabaseInspection(
      "Phase 2: Customer Onboarding",
      "Step 3: Feature Access Verification Complete",
    );
  }
...
async function runPhase3_TrialToPurchase(subscrio: Subscrio) {
  ...
  console.log(
    '📥 Input: subscrio.subscriptions.updateSubscription("acme-subscription", {',
  );
  ...
  const convertedSubscription =
    await subscrio.subscriptions.getSubscription("acme-subscription");
  console.log(
    "📄 Output: Converted Subscription DTO:",
    JSON.stringify(convertedSubscription, null, 2),
  );
  ...
  if (isInteractiveMode) {
    await promptForDatabaseInspection(
      "Phase 3: Trial to Purchase",
      "Step 1: Trial Conversion Complete",
    );
  }
...
async function runPhase4_PlanUpgrade(subscrio: Subscrio) {
  ...
  if (upgradedSubscription) {
    console.log(
      "📄 Output: Upgraded Subscription DTO:",
      JSON.stringify(upgradedSubscription, null, 2),
    );
...
async function runPhase5_FeatureOverrides(subscrio: Subscrio) {
  ...
  if (isInteractiveMode) {
    await promptForDatabaseInspection(
      "Phase 5: Feature Overrides",
      "Step 1: Temporary Override Added",
    );
  }
  ...
  if (isInteractiveMode) {
    await promptForDatabaseInspection(
      "Phase 5: Feature Overrides",
      "Step 2: Permanent Override Added",
    );
  }
...
async function runPhase6_SubscriptionRenewal(subscrio: Subscrio) {
  ...
  printInfo(
    "Subscription renews - temporary overrides are cleared, permanent ones remain",
    1,
  );

  // Clear temporary overrides (simulating renewal)
  console.log(
    '📥 Input: subscrio.subscriptions.clearTemporaryOverrides("acme-subscription")',
  );
  ...
  const renewedSubscription =
    await subscrio.subscriptions.getSubscription("acme-subscription");
  console.log(
    "📄 Output: Renewed Subscription DTO:",
    JSON.stringify(renewedSubscription, null, 2),
  );
  ...
  printInfo(
    `Gantt charts: ${hasGanttCharts ? "Enabled" : "Disabled"} (permanent override remains)`,
    1,
  );
...
async function runPhase7_DowngradeToFree(subscrio: Subscrio) {
  ...
  printInfo(
    "Customer cancels the professional subscription effective immediately",
    1,
  );
  ...
  if (cancelledSubscription) {
    console.log(
      "📄 Output: Cancelled Subscription DTO:",
      JSON.stringify(cancelledSubscription, null, 2),
    );
    printInfo(`Status: ${cancelledSubscription.status}`, 1);
    printInfo(
      `Cancellation date: ${cancelledSubscription.cancellationDate ? new Date(cancelledSubscription.cancellationDate).toLocaleDateString() : "N/A"}`,
      1,
    );
    printInfo(
      `Current period end: ${cancelledSubscription.currentPeriodEnd ? new Date(cancelledSubscription.currentPeriodEnd).toLocaleDateString() : "N/A"}`,
      1,
    );
    printInfo(
      `The cancellation date is now, so paid access has ended. Use a future cancellation date to schedule cancellation.`,
      1,
    );
  }
  ...
  if (isInteractiveMode) {
    await promptForDatabaseInspection(
      "Phase 7: Customer Cancellation",
      "Step 1: Subscription Cancelled",
    );
  }
  ...
  printInfo(
    "After the paid plan lapses, the customer opts into the free tier manually",
    1,
  );
  ...
  console.log(
    "📄 Output: Free Subscription DTO:",
    JSON.stringify(freeSubscription, null, 2),
  );
  ...
  if (isInteractiveMode) {
    await promptForDatabaseInspection(
      "Phase 7: Customer Downgrade",
      "Step 2: Free Plan Subscription Created",
    );
  }
  ...
  printInfo(
    "Verify customer has access to free plan features after downgrading",
    1,
  );
  ...
  printInfo(
    `Gantt charts: ${hasGanttCharts ? "Enabled" : "Disabled"} (not available on free plan)`,
    1,
  );
  printInfo(
    `Custom branding: ${hasCustomBranding ? "Enabled" : "Disabled"} (not available on free plan)`,
    1,
  );
  printInfo(
    `API access: ${hasApiAccess ? "Enabled" : "Disabled"} (not available on free plan)`,
    1,
  );

  // Show resolved feature values
  console.log(
    "📄 Output: Resolved Feature Values (Free Plan):",
    JSON.stringify(
      {
        maxProjects,
        maxUsers,
  ...
  if (isInteractiveMode) {
    await promptForDatabaseInspection(
      "Phase 7: Customer Downgrade",
      "Step 3: Free Plan Features Verified",
    );
  }
...
async function promptDemoStart(automated: boolean = false) {
  if (automated) {
    ...
    console.log(
      "This demo will delete existing demo entities and then create the following entities:",
    );
    console.log("");
    ...
    console.log(
      "💳 BILLING CYCLES: free-monthly, starter-monthly, starter-annual, professional-monthly, professional-annual, enterprise-monthly, enterprise-annual",
    );
    console.log("👤 CUSTOMERS: acme-corp");
    console.log(
      "🔄 SUBSCRIPTION LIFECYCLE: trial → purchase → upgrade → renewal → cancellation → downgrade",
    );
    console.log("");
    console.log("⚠️  Please ensure you have a dedicated test database or");
    console.log(
      "   are prepared to manually clean up these entities after the demo.",
    );
    console.log("");
  ...
  console.log(
    "💳 BILLING CYCLES: starter-monthly, starter-annual, professional-monthly, professional-annual, enterprise-monthly, enterprise-annual",
  );
  console.log("👤 CUSTOMERS: acme-corp");
  console.log(
    "🔄 SUBSCRIPTION LIFECYCLE: trial → purchase → upgrade → renewal",
  );
  console.log("");
  console.log("⚠️  Please ensure you have a dedicated test database or");
  console.log(
...
async function cleanupDemoEntities(subscrio: Subscrio) {
  ...
  await safeDelete(
    `DELETE FROM subscrio.customers WHERE key = 'acme-corp'`,
    "Deleting customers",
  );
  await safeDelete(
  ...
  await safeDelete(
    `DELETE FROM subscrio.products WHERE key = 'projecthub'`,
    "Deleting products",
  );
