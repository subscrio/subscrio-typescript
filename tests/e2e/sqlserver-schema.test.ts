...
describe('SQL Server schema smoke', () => {
  ...
  sqlServerTest('installs and drops Subscrio schema on a live SQL Server instance', () => {
    ...
    runSql(
      'master',
      `IF DB_ID('${DB_NAME}') IS NOT NULL BEGIN ALTER DATABASE [${DB_NAME}] SET SINGLE_USER WITH ROLLBACK IMMEDIATE; DROP DATABASE [${DB_NAME}]; END; CREATE DATABASE [${DB_NAME}];`
    );
